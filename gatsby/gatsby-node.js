...
(async () => {
  ...
  const listResult = await page.evaluate(() => {
	...
	for (let film of listResult) {
		await page.goto('http://est.hu/mozi/filmek_a_heten/#filmkereso/film=' + film.estId);
		const screeningResult = await page.evaluate(() => {
			let res = [];
			const focella = document.querySelectorAll('.fo_cella a');
			for (let i = 0; i < focella.length; i++) {
				let screening = {};
				screening.location = focella[i].textContent;
				res.push(screening);
			}
			return res;
		})
		film.screening = screeningResult;
	}
